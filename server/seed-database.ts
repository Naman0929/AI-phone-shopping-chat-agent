...
async function setupDatabaseAndCollection(): Promise<void>{
    console.log("Setting up database and collecton...")
...
async function generateSyntheticData(): Promise<Item[]> {
    const prompt = `You are a helpful assistant that generates mobile phones model data. 
    Generate 50 mobile phone models. Each record should include the following fields: 
    item_id, item_name, item_description, brand, manufacturer_address, prices, categories, 
    user_reviews, notes. Ensure variety in the data and realistic values. Make sure to generate data
    considering you would be catering Indian audience.
...
async function createItemSummary(item: Item): Promise<string> {
    return new Promise((resolve) => {
        ...
        const price = `At full price it costs: ${item.prices.full_price} INR, 
        On sale it costs: ${item.prices.sale_price} INR`
...
async function seedDatabase(): Promise<void> {
    try {
        ...
        console.log("You successfully connceted to MongoDB!")
        ...
        console.log("Cleared existing data from item collection")
